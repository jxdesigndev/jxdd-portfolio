...
# Using robust regex to find the block
pattern = r'/\* ─+[\s\S]*?THE HUB \(NEW ABOUT PAGE\)[\s\S]*?─+ \*/[\s\S]*?(?=/\* ─+[\s\S]*?INTERIOR VIEWS)'
...
.dbx-bento-container {
  ...
  z-index: 30;
  pointer-events: none; /* Let fixed elements breathe until grid is fully opaque */
}
...
.dbx-bento-grid {
  ...
  width: 100%;
  height: 100%;
  gap: 1px; /* The 1px border lines */
  background: rgba(100, 255, 100, 0.2); /* Grid line color */
...
.dbx-title {
  ...
  color: var(--white);
  line-height: 1.1;
  overflow: hidden; /* For masking text */
}
...
if re.search(pattern, css):
    ...
    print("Pattern not found. Debugging pattern:")
    print("Looking for THE HUB (NEW ABOUT PAGE)")
