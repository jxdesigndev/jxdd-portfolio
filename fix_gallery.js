...
const processNew = `      project.process_image_urls.forEach((url, i) => {
        const frame = document.createElement('div');
        frame.className = 'screenshot-frame frame-portrait'; // Default
        frame.style.transitionDelay = \`\${i * 0.1}s\`;
        const img = document.createElement('img');
        img.alt = \`Process \${i + 1}\`;
        img.loading = 'lazy';
...
const screenOld = `      const framePromises = project.screenshot_urls.map((url, i) => new Promise(resolve => {
        const frame = document.createElement('div');
        frame.className = 'screenshot-frame';
        frame.style.transitionDelay = \`\${i * 0.1}s\`;
        const img = document.createElement('img');
...
const screenNew = `      project.screenshot_urls.forEach((url, i) => {
        const frame = document.createElement('div');
        frame.className = 'screenshot-frame frame-portrait'; // Default
        frame.style.transitionDelay = \`\${i * 0.1}s\`;
        const img = document.createElement('img');
...
code = code.replace(screenOld, screenNew);
