...
(async () => {
  ...
  try {
    ...
    // Wait for JS to initialize (Three.js takes a moment)
    ...
    // Clear initial load jitter
    await page.evaluate(() => { window.fpsData.idle = []; window.fpsData.scroll = []; });
    ...
    await page.evaluate(() => {
        ...
        function scrollLoop() {
            y += 200;
            window.scrollTo(0, y);
            if (y < 4000) requestAnimationFrame(scrollLoop);
        }
        scrollLoop();
    });
